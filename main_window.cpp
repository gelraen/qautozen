...
QLayout* AutozenWindow::createBeatSlider() {
	...
	connect(beat_slider_, SIGNAL(sliderMoved(int)), beat_label, SLOT(setNum(int)));
	connect(beat_slider_, SIGNAL(sliderMoved(int)), sound_manager_, SLOT(setBeat(int)));
	return l;
}
...
QLayout* AutozenWindow::createBaseSlider() {
	...
	connect(base_slider_, SIGNAL(sliderMoved(int)), base_label, SLOT(setNum(int)));
	connect(base_slider_, SIGNAL(sliderMoved(int)), sound_manager_, SLOT(setBase(int)));
	return l;
}
...
QLayout* AutozenWindow::createVolumeSlider() {
	...
	connect(volume_slider_, SIGNAL(sliderMoved(int)), volume_label, SLOT(setNum(int)));
	connect(volume_slider_, SIGNAL(sliderMoved(int)), sound_manager_, SLOT(setVolume(int)));
	return l;
}
