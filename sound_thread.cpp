...
struct CurrentState {
 ...
 public:
  CurrentState(int sampleRate)
      : volume_(100), base_(300), beat_(25), sample_rate_(sampleRate) {
  ...
  void IncerementCurtimes() {
    ...
    for (int i = 0; i < harmonics_; i++) {
      harmonic_curtime_right_[i] = fmod(
          harmonic_curtime_right_[i] + base_ * pow(2, i) + beat_, sample_rate_);
    }
  }
...
SoundManager::~SoundManager() { delete state_; }
...
void SoundManager::initOut() {
  ...
  out_->setBufferSize(kSampleRate * kBufferLenMs / 1000 * kSampleSize / 8 *
                      kChannelCount);
  out_->setNotifyInterval(kBufferLenMs / 2);

  connect(out_, SIGNAL(stateChanged(QAudio::State)),
          SLOT(onOutStateChanged(QAudio::State)));
...
void SoundManager::start() {
  ...
  // qDebug() << "State: " << out_->state() << ", Error: " << out_->error() <<
  // ", FreeBytes: " << out_->bytesFree();
...
void SoundManager::fillBuffer(CurrentState *state, QIODevice *out,
                              int freeBytes) {
